...
function setMaps(data) {
  ...
  let url = 'https://osu.ppy.sh/api/get_user_best?k=<key>&u=<ign>'.replace('<key>', data.key).replace('<ign>', data.stats.username);
  // setup request
  ...
  request.onreadystatechange = function() {
    ...
    if(request.readyState === 4 && request.status === 200)
      // save data
      data.maps = JSON.parse(request.responseText).slice(0, 4);
  };
  request.send();
}
