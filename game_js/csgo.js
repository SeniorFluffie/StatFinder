...
function simplifyCsgo(data) {
  ...
  let bestStats = [{prop: 'best_weapon', prefix: 'total_kills_', escape: true, restrict: ['total_kills_against_zoomed_sniper', 'total_kills_elite', 'total_kills_enemy_blinded',
  'total_kills_enemy_weapon', 'total_kills_headshot', 'total_kills_knife_fight']},
  {prop: 'best_hit', prefix: 'total_hits_', escape: true}, {prop: 'best_wins', prefix: 'total_wins_map_'}, {prop: 'best_map', prefix: 'total_rounds_map_'}];
  // iterate through the stats
  for(let stat of bestStats) {
    // intialize text
    let text, best;
    // find best stat
    stat.restrict ? best = findBest(stats, stat.prefix, stat.restrict) : best = findBest(stats, stat.prefix, undefined);
    // concat string
...
function findBest(data, prefix, restrict) {
  ...
  for(let prop in data)
    ...
    if(prop !== prefix && prop.includes(prefix) && (restrict === undefined || !restrict.includes(prop)))
...
function csgoTable(data) {
  ...
  [{title: 'Weapon Kills', key: 'best_weapon'}, {title: 'Shots Hit', key: 'best_hit'}, {title: 'Rounds Won', key: 'best_wins'},
  {title: 'Rounds Played', key: 'best_map'}],
